...
always walk only a single block for each letter
 (direction) and you know it takes you one minute to
 traverse one city block, so create a function that 
 will return true if the walk the app gives you will 
 take you exactly ten minutes (you don't want to be
   early or late!) and will, of course, return you 
...
containing a random assortment of direction letters
 ('n', 's', 'e', or 'w' only). It will never give
   you an empty array (that's not a walk, that's 
   standing still!).
...
def ReachedToSamePoint(arr):
    #north and south are opposite directions
    #east and west are opposite directions
    #creating dict for the same

    if len(arr)!=10: return False
    c=Counter(arr)

    return c['w']==c['e'] and c['n']==c['s']

def main():
    str_arr = input('Enter the directions sep by ,:')
    arr=str_arr.split(',')

    arrived = ReachedToSamePoint(arr)
    log.info(f'arrived: {arrived}')
